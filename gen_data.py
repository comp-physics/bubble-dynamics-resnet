...
#--------------------------------------------------------
train_data = np.zeros((n_train, n_steps + 1, n_input))
...
for idx in range(n_train):
    #----------------------------------
    sound = bub.SoundWave(amp_range, freq_range, n_wave)
    ...
    train_data[idx, 0, :n_output] = y_init
    train_data[idx, 1:, :n_output] = sol.y.T[0:n_steps]
    train_data[idx, :, n_output:] = np.column_stack((P.reshape(n_steps+1,1), Pdot.reshape(n_steps+1,1)))
...
for k in range(n_delta):
    ...
    slice_data = np.zeros((N, slice_size + 1, n_input))
...
#--------------------------------------------------------
val_data = np.zeros((n_val, n_steps + 1, n_input))
...
for idx in range(n_val):
    ...
    # ----------------------------------
    sol = solve_ivp(bubble.rhs_rp, t_span=[0.0, t_final+dt], y0=y_init, args=(sound,), t_eval=t_space, method='LSODA', rtol=rel_tol, atol=abs_tol)
    val_data[idx, 0, :n_output] = y_init
    val_data[idx, 1:, :n_output] = sol.y.T[0:n_steps]
    val_data[idx, :, n_output:] = np.column_stack((P.reshape(n_steps+1,1), Pdot.reshape(n_steps+1,1)))
...
#--------------------------------------------------------
test_data = np.zeros((n_test, n_steps + 1, n_input))
...
for idx in range(n_test):
    ...
    # ----------------------------------
    sol = solve_ivp(bubble.rhs_rp, t_span=[0.0, t_final+dt], y0=y_init, args=(sound,), t_eval=t_space, method='LSODA', rtol=rel_tol, atol=abs_tol)
    test_data[idx, 0, :n_output] = y_init
    test_data[idx, 1:, :n_output] = sol.y.T[0:n_steps]
    test_data[idx, :, n_output:] = np.column_stack((P.reshape(n_steps+1,1), Pdot.reshape(n_steps+1,1)))
