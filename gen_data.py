...
for j in j_samples:
    ...
    axs[j].plot(t, P_t, color='tab:red', label='$P(t)$')
    axs[j].plot(t, R_t, color='tab:blue', label='$R(t)$')
    # plt.plot(t, Rdot_t, color='tab:green', label='$\dot{R}(t)$')
    axs[j].legend(fontsize=30, loc='upper center', ncol=3, bbox_to_anchor=(0.5, 1.2))
    axs[j].xlabel('t / $t_0$')
    axs[j].ylabel('R / $R_0$')
    parameters = '$P(t=0)=$ {0:.3f}\n $R(t=0)=$ {1:.3f}\n $ \dot{{R}} (t=0)=$ {2:.3f}\n'.format(P_t[0], R_t[0], Rdot_t[0])
    props = dict(boxstyle='round', facecolor='wheat', alpha=0.25)
    axs[j].text(0.6*tmax, max(R_t), parameters, fontsize=14, verticalalignment='top', bbox=props)
