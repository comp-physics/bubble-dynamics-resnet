...
class ResNet(torch.nn.Module):
    ...
    def forward(self, x_init):
        ...
        """
        return x_init[:, :self.n_outputs] + self._modules['increment'](x_init)
    ...
    def uni_scale_forecast(self, x_init, n_steps, y_known=None):
        ...
        while cur_step <= n_steps: # archived code: "cur_step < n_steps + self.step_size"
            if y_known is None:  # (Scott Sims) adapted for when n_inputs > n_outputs
                x_next = self.forward(x_prev)  # x(i) = x(i-1) + f( x(i-1) )
            else:
                x_model = self.forward(x_prev)
                x_next = torch.column_stack((x_model, y_known[:, cur_step, :]))
    ...
    def calculate_loss(self, x_init, ys, w=1.0):
        ...
        for t in range(n_steps):
            if y_known is None:  # (Scott Sims) adapted for when n_inputs > n_outputs
                ...
                y_model = self.forward(y_prev)
                y_next = torch.column_stack((y_model, y_known[:, t, :])) # ENSURE THAT t IS ACCRUATE BY CHECKING utils.py
            y_preds[:, t, :] = y_next
            y_prev = y_next
...
def vectorized_multi_scale_forecast(x_init, n_steps, models, y_known=None, key=False):
    ...
    """
    # sort models by their step sizes (in decreasing order)
    step_sizes = [model.step_size for model in models]
    models = [model for _, model in sorted(zip(step_sizes, models), reverse=True)]

    # we assume models are sorted by their step sizes (decreasing order)
    n_test, n_outputs = x_init.shape                         # n_test = number of x(0) values to test; n_inputs = dimension of each x0
    if y_known is not None:
        print('------------------')
        print('y_known.shape = {}'.format(y_known.shape))
        print('n_steps + 1 = {}'.format(n_steps+1))
        assert (n_steps+1) == y_known.shape[1]
        n_known = y_known.shape[2]
        n_inputs = n_outputs + n_known
    #-------------------------------------------------------------
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    indices = list()
    # archived code: extended_n_steps = n_steps + models[0].step_size
    # archived code: preds = torch.zeros(n_test, extended_n_steps + 1, n_inputs).float().to(device)
    extended_n_steps = n_steps
    ...
    if y_known is None:
        ...
        preds[:, 0, :] = torch.column_stack((x_init, y_known[:, 0, :]))
    indices.append(0)
    total_step_sizes = n_steps
    for model in models:                                              # for each model (largest 'step_size' first)
        n_forward = round(total_step_sizes/model.step_size)-1             # pick how many steps forward (rounded down)
        ...
        model_idx += int(1)
        print("model index = {}".format(model_idx))# (Scott Sims) used when optional argument 'key' == True
        for t in range(n_forward):                                    # for t-steps forward
            ...
            if y_known is None:  # (Scott Sims) adapted for when n_inputs > n_outputs
                ...
                y_model = model(y_prev)
                y_known_temp = y_known[:, shifted_indices, :].reshape(-1, n_known)
                y_next = torch.column_stack((y_model, y_known_temp))   # y(i) = y(i-1) + f( y(i-1) )
            #-------------------------
            # (Scott Sims)
            if(key == True):
        ...
        total_step_sizes = model.step_size                        # used to set n_forward for next model, e.g. n_forward = int( (2^6-1) / 2^5 ) = 1
    ...
    last_model = models[-1]
    while last_idx < n_steps: # archived code: last_idx < n_steps
        last_idx += last_model.step_size
        if y_known is None:  # (Scott Sims) adapted for when n_inputs > n_outputs
            y_next = last_model(y_prev)  # y(i) = y(i-1) + f( y(i-1) )
        else:
            y_known_temp = y_known[:, last_idx, :].reshape(-1, n_known)
            y_model = model(y_prev)
            y_next = torch.column_stack((y_model, y_known_temp))  # y(i) = y(i-1) + f( y(i-1) )
