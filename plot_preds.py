...
if num_arg != 6:
    msg = "Expected 6 arguments but counted {}.".format(num_arg)
    msg = msg + "\n| system | dt | k_max | num_inputs | num_layers | layer_size |"
    sys.exit(msg)
...
for step_size in step_sizes:
    models.append(torch.load(os.path.join(model_dir, 'model_D{}_{}_{}x{}.pt'.format(step_size,num_inputs,num_layers,layer_size)), map_location='cpu'))
num_k = len(models)
...
figs_folder = 'figs_' + date_time + '_' + system + '_kmax={}'.format(k_max) + '_tmax={:.2f}'.format(dt*n_steps) + '_dt={:.2f}'.format(dt)
figs_dir = os.path.abspath( os.path.join(parent_dir, figs_folder) )
...
for model in models:
    ...
    axs[k].plot(t, test_data[idx, 0:n_steps, 0], linestyle='-', color='lightgray', linewidth=10, label='x(t)' )
    axs[k].plot(t, test_data[idx, 0:n_steps, 1], linestyle='-', color='gray', linewidth=10, label='y(t)')
...
#-------------------------------------------------------------
plt.plot(t, test_data[idx, 0:n_steps, 0], color='gray', linestyle='-', linewidth=10, label='x(t)' )
plt.plot(t, test_data[idx, 0:n_steps, 1], color='lightgray', linestyle='-', linewidth=10, label='y(t)')
#plt.plot(t, test_data[idx, 0:n_steps, 2], color='darkgray', linestyle='-', linewidth=10, label='$\dot{r}(t)$')
#-------------------------------------------------------------
#plt.plot(t, P, color='black', linestyle='--', linewidth=5, label='learned')
#plt.plot(t, R, color='black', linestyle='--', linewidth=5)
...
colors = iter( reversed(plt.cm.rainbow(np.linspace(0, 1, len(models))) ) ) 
dot_sizes = iter( reversed( np.linspace(dot_max,dot_min,len(models)) ) )
